...
**MentorBoxAI** is an AI-powered learning assistant that transforms complex technical and educational concepts into engaging 3Blue1Brown-style animated explainers. Designed for the **[Student Track] AI for Learning & Developer Productivity** challenge, the platform helps people learn faster and work smarter by automatically generating visual content from simple text prompts.
...
- **Enhances understanding** through 3Blue1Brown-style mathematical and scientific visualizations
...
1. THE System SHALL provide FastAPI-based REST endpoints for video generation requests via `backend_local.py`
2. WHEN receiving requests, THE System SHALL integrate with OpenRouter API using Qwen 2.5 72B model for advanced reasoning
3. WHEN processing videos, THE System SHALL use Manim Community Edition v0.19 for professional-quality animation rendering
...
6. THE System SHALL be configurable for different LLM backends (OpenRouter) to support various organizational needs
...
5. IF generation fails after all retry attempts, THEN THE System SHALL provide clear failure notification with alternative learning resources
6. THE System SHALL implement automatic error recovery via LLM reviewer to maintain learning continuity

### Requirement 9: Cross-Platform Learning Accessibility

**User Story:** As a learner or developer working across different environments, I want flexible deployment options that work on my preferred platform, so that I can access the learning assistant wherever I work most effectively.

#### Acceptance Criteria

1. THE System SHALL support Windows PowerShell deployment via `run-local.ps1` script for Windows developers
2. THE System SHALL support WSL/Linux deployment with setup instructions in `wsl-run.txt` for Unix-based workflows
3. THE System SHALL provide cloud-ready architecture with stateless API design for institutional deployment
4. THE System SHALL maintain file structure compatibility with existing development workflows
